 ...
 */
class EfetivacaoForm extends TPage
{
    protected $form; // form
    ...
    public function __construct()
    {
        ...
        */
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_efetivicao');
        $this->form->setFormTitle('Validação - Efetivação do Contrato');
        ...
        $produto_id = new TDBCombo('produto_id', 'erphouse', 'Produto', 'id', 'produto', null, TCriteria::create( ['ativo' => 'sim'] )); //Adicionar aqui um parâmetro para filtrar apenas os produtos do fornecedor selcionado
        ...
        $Valida_valor = new TNumeric('Valida_valor', 2, ',', '.');
        $renova_valor = new TNumeric('renova_valor', 2, ',', '.');
        $valida_prog = new TDate('valida_prog');
        ...
        TRadioGroup::disableField('form_efetivicao', 'gera_viagem');
        ...
        TButton::disableField('form_efetivicao', 'botaendereco');
        ...
        $Valida_valor->setSize('100%');
        $renova_valor->setSize('100%');
        $valida_prog->setSize('100%');

        $valida_prog->setMask('dd/mm/yyyy');
        $valida_prog->setDatabaseMask('yyyy-mm-dd');
        ...
        if (!empty($id))
        {
            ...
            $periodicidade_id->setEditable(FALSE);
            $melhor_dia->setEditable(FALSE);
            //$efetivacao->setEditable(FALSE);
            //$forma_pagamento->setEditable(FALSE);
            //$detail_produto->setEditable(FALSE);
            //$detail_valor->setEditable(FALSE);
            //$endereco_contrato_id->setEditable(FALSE);
        ...
        //Botão para reagendar coleta - Atualiza a data do agendamento de coleta
        $coleta = TButton::create('coleta', [$this, 'onReagendar'], 'Reagendar Efetivação', 'fa:plus-circle green');
        $coleta->getAction()->setParameter('static','1');
        ...
        $row->layout = ['col-sm-3', 'col-sm-3', 'col-sm-2', 'col-sm-2', 'col-sm-1'];
        ...
        //Botão para parcelar a programação dos depósitos
        $botaparcela = TButton::create('botaparcela', [$this, 'ParcelarDeposito'], 'Dividir Depósitos', 'fas:check green');
        $botaparcela->getAction()->setParameter('static','1');

        //[ new TLabel('Selecione endereço'), $tipo_endereco ]
        $row = $this->form->addFields(  [ $botacartao, $forma_pagamento ],
                                        [ new TLabel('Data Programada'), $valida_prog ],
                                        [ new TLabel(''), $botaparcela ]
                                        //[ new TLabel('Selecione um Endereço'), $endereco_contrato_id ]
                                        );
        $row->layout = ['col-sm-2', 'col-sm-2', 'col-sm-1'];
       
        //parâmetros do botão adicionar endereços
        $botaendereco->setAction(new TAction(array($this, 'AdicionarEndereco')),"Selecione um Endereço");
        ...
        $btValida = TButton::create('btValida', [$this, 'onSave'], 'Validar Contrato', 'fas:check green');
        $btValida->getAction()->setParameter('static','1');

        $row = $this->form->addFields(  [ new TLabel('Valor Efetivado'), $Valida_valor ],
                                        [ new TLabel('Valor Renovado'), $renova_valor ],
                                        [ new TLabel(''), $btValida ]
                                        );
        $row->layout = ['col-sm-2', 'col-sm-2', 'col-sm-1']; 
        ...
        //Botoões do form
        $this->form->addHeaderActionLink('Voltar para a Lista',  new TAction(array('EfetivacaoList','onReload')),  'fa:times red' );
        //$btn = $this->form->addAction('Gerar Guia', new TAction(array($this, 'onGenerate')), 'far:check-circle green');
        //$btn->class = 'btn btn-sm btn-primary';
        //$this->form->addActionLink('Limpar',  new TAction([$this, 'onClear']), 'fa:eraser red');
        $this->form->addActionLink('Voltar para a Lista', new TAction(array('EfetivacaoList','onReload')),  'fa:times red' );
    ...
    //Função que registra o reagemento de data e atualiza a data de efetivaçção na tabela contrato
    public function onReagendar($param)
    {
        try
        {
            ...
            TForm::sendData('form_efetivicao', (object) ['id' => $master->id]);

            TRadioGroup::enableField('form_efetivicao', 'gera_viagem'); //Libera o botão para que possa ser adicionado um endereço
            TButton::enableField('form_efetivicao', 'botaendereco');
            
            new TMessage('info', 'Reagendamento Registrado com Sucesso!');
        }
    ...
    public function ValidaContrato($param)
    {
        try
        {
            // open a transaction with database
            ...
            $master->status = 'Aguardando Pagamento';
            $master->store();
            Transaction::close(); // close the transaction
    ...
    function onGenerate($param)
    {
        ...
        {   
            ...
            $pdf->fromXml('app/output/logistica/local_coleta.pdf.xml');

            //Conexões com o banco para carregar os nomes
            TTransaction::open('erphouse');

            //Total da Doação
            $total = ViewContratos::where('contrato_id', '=', $data->id)
                                    ->where('cliente_id', '=', $data->cliente_id)
                                    ->where('efetivacao', '=', $data->efetivacao)
                                    ->sumBy('total');
            
            $total =  round($total * 1, 3);
            ...
            //Nome do beneficiado            
            $vFornecedor = Pessoa::find($data->fornecedor_id);
            if ($vFornecedor instanceof Pessoa)
            {
                $data->fornecedor_id = $vFornecedor->nome;
            }  
            //Fim benecifiado

             //Nome do doador            
             $vDoador = Cliente::find($data->cliente_id);
             if ($vDoador instanceof Cliente)
             {
                $data->cliente_id = $vDoador->nome;
             }  
             //Fim doador
            ...
            $data_efet = new DateTime($data->efetivacao);
            $dt_result = $data_efet->format('d/m/Y');

            $pdf->replace('{contrato}', $data->id);
            ...
            $file = 'app/output/logistica/local_coleta.pdf';
            ...
            if (!file_exists($file) OR is_writable($file))
            {
                ...
                $window = TWindow::create('Guia de Coleta', 0.9, 0.91);
    ...
    public function onDetailAdd( $param )
    {
        try
        {
            ...
            TForm::sendData( 'form_efetivicao', $data, false, false );
    ...
    public static function onDetailEdit( $param )
    {
        ...
        TDBCombo::reloadFromModel('form_efetivicao', 'detail_produto', 'erphouse', 'Produto', 'id', 'produto', null, $criteria, TRUE);
    ...
    public function onSave($param)
    {
        try
        {
            ...
            $master->system_user_id = TSession::getValue('userid');
            $master->ativo = 'sim';
            $master->status = 'Efetivado';
            $master->store();
            
            //Registra a validação do contrato
            Efetivacao::where('contrato_id', '=', $master->id)->delete();
                $efet = New Efetivacao;
                $efet->contrato_id = $master->id;                
                $efet->valor = (float) str_replace(['.', ','], ['', '.'],$param['Valida_valor']);
                $efet->valor_renovado = (float) str_replace(['.', ','], ['', '.'],$param['renova_valor']);
                $efet->data_efet = $master->efetivacao;
                $efet->forma_pagamento = $master->forma_pagamento;

                //Valição para o tipo de pagamento selecionado
                //Crédito: A data programada será sempre 30 dias do dia da efetivaçaõ
                if ($master->forma_pagamento == 'cartao de credito')
                {
                    $data->valida_prog = date('Y-m-d', strtotime("+30 days",strtotime($data->valida_prog)));
                }

                if ($master->forma_pagamento == 'dinheiro')
                {
                    $data->valida_prog = date('Y-m-d');
                }                
                
                $efet->data_prog = $data->valida_prog;
                $efet->store();            
            //Fim de Efetivação do contrato


            //Gerar novo contrato após a efetivação seguindo as regras de renovação do contrato original
            $Nmaster = new Contrato;
            $Nmaster->fromArray( (array) $data);
            if($Nmaster->dt_inicio == null)
            {
                $Nmaster->dt_inicio = date('Y-m-d');
            }
            $Nmaster->id = NULL;
            $Nmaster->system_user_id = TSession::getValue('userid');
            $Nmaster->ativo = 'sim';
            $Nmaster->status = 'Aguardando Contato';
            
            //Validação da data para o próximo contato
            $vPeriodo = Periodicidade::find($param['periodicidade_id']);                    
                if ($vPeriodo instanceof Periodicidade)
                {
                    $vPeriodo = $vPeriodo->repete;
                    
                }
            $dta_prox = date('Y-m-d', strtotime("+{$vPeriodo} month",strtotime($master->efetivacao)));    
            $dta_prox = explode("-", $dta_prox);
            list($ano, $mes, $dia) = $dta_prox;
            $dta_prox = "$ano-$mes-$master->melhor_dia";            
            $Nmaster->efetivacao = $dta_prox;
            //Fim da validação da data para o próximo contato
            //-----------------------------------------------------------------------------------------

            $Nmaster->store();        
            
            $items  = ContratoItem::where('contrato_id', '=', $master->id)->load();
            foreach( $items as $item )
            {
                //Valida se o produto é renovável
                $vRenova = Produto::find($item->produto_id);                    
                if ($vRenova instanceof Produto)
                {
                    $vRenova = $vRenova->renova;
                }
                
                if( $vRenova == 'sim')
                {
                    $Ndetail = new ContratoItem;
                    $Ndetail->produto_id  = $item->produto_id;
                    $Ndetail->valor  = $efet->valor_renovado;
                    $Ndetail->contrato_id = $Nmaster->id;
                    $Ndetail->quantidade = 1;
                    $Ndetail->total = $efet->valor_renovado;
                    $Ndetail->data_contrato = date('Y-m-d');                    
                    $Ndetail->status = 'Aguardando Contato';
                    $Ndetail->store();
                }
            }
            //Fim da geração do novo contrato
            ...
            new TMessage('info', 'Contrato Efetivado com Sucesso!');
        }
        catch (Exception $e) // in case of exception
        {
            new TMessage('error', $e->getMessage());
    ...
    public static function onGeraViagem($param)
    {
        ...
        {
            TCombo::enableField('form_efetivicao', 'endereco_contrato_id');
        ...
        {
            TCombo::disableField('form_efetivicao', 'endereco_contrato_id');
    ...
    public static function onListaProduto($param)
    {
        try
        {
            ...
            {
                TDBCombo::clearField('form_efetivicao', 'detail_produto');
            }
    ...
    public function AdicionarEndereco($param)
    {
        ...
        TSession::setValue('form_efetivicao',$data);   
    ...
    public static function ListaEnderecos($param)
    {
        try
        {
            ...
            if (!empty($param))
            {
                ...
                // formname, field, database, model, key, value, ordercolumn = NULL, criteria = NULL, startEmpty = FALSE
                TDBCombo::reloadFromModel('form_efetivicao', 'endereco_contrato_id', 'erphouse', 'EnderecoContrato', 'id', 'tipo', null, $criteria, TRUE);
            }
            else
            {
                TDBCombo::clearField('form_efetivicao', 'endereco_contrato_id');
            }
    ...
    public function ParcelarDeposito($param)
    {
        $data = $this->form->getData();
        $this->form->validate();
        $data->id = $param['id'];
        $data->Valida_valor = $param['Valida_valor'];
        
        TSession::setValue('form_Contrato',$data);   
             
        $script = "__adianti_load_page('engine.php?class=ParcelaForm&method=onEdit&id={$data->id}&key={$data->id}&valor={$data->Valida_valor}');";
        TScript::create($script);
    }
